"""Build the detailed Steamtek_C002 prototype from the approved master and rig."""
...
def main():
    ...
    cyan_bsdf = cyan.node_tree.nodes.get("Principled BSDF")
    if cyan_bsdf is not None and "Emission Color" in cyan_bsdf.inputs:
        cyan_bsdf.inputs["Emission Color"].default_value = (0.01, 0.38, 0.70, 1.0)
        cyan_bsdf.inputs["Emission Strength"].default_value = 3.0

    def hose(name, bone_name, points, radius=0.014, mat=rubber):
    ...
    for x in (-0.14, 0.14):
        cube_part(f"C002_HarnessStrap_{x:+.2f}", rig, "DEF-spine.003",
                  chest + Vector((x, -0.145, -0.01)),
                  (0.022, 0.012, 0.19), facing_root, collection, copper)
    cube_part("C002_HarnessCrossbar", rig, "DEF-spine.003", chest + Vector((0, -0.158, 0.02)),
              (0.17, 0.012, 0.018), facing_root, collection, copper)
    for x in (-0.19, 0.19):
        cube_part(f"C002_BeltPouch_{x:+.2f}", rig, "DEF-spine.001",
                  abdomen + Vector((x, -0.135, -0.115)),
                  (0.055, 0.035, 0.06), facing_root, collection, jacket)
    ...
    cube_part("C002_MaskCenter", rig, "DEF-spine.006", head + Vector((0, -0.178, -0.015)),
              (0.035, 0.018, 0.055), facing_root, collection, steel)
    cube_part("C002_CapLamp", rig, "DEF-spine.006", head + Vector((0, -0.155, 0.125)),
              (0.032, 0.018, 0.022), facing_root, collection, cyan)
    ...
    for z in (-0.09, -0.03, 0.03, 0.09):
        cube_part(f"C002_GauntletBand_L_{z:+.2f}", rig, "DEF-forearm.L",
                  forearm_l + Vector((0, -0.142, z)),
                  (0.105, 0.016, 0.012), facing_root, collection, steel)
    ...
    cube_part("C002_WrenchHead_R", rig, "DEF-spine.001", abdomen + Vector((-0.34, 0.015, -0.315)),
              (0.06, 0.035, 0.025), facing_root, collection, steel)
    ...
    cube_part("C002_BackFrameTop", rig, "DEF-spine.003", chest + Vector((0, 0.265, 0.19)),
              (0.22, 0.025, 0.018), facing_root, collection, steel)
    cube_part("C002_BackFrameBottom", rig, "DEF-spine.003", chest + Vector((0, 0.265, -0.17)),
              (0.22, 0.025, 0.018), facing_root, collection, steel)
    hose("C002_Hose_L", "DEF-spine.003", [
        chest + Vector((0.11, 0.30, 0.13)),
        chest + Vector((0.24, 0.24, 0.02)),
        chest + Vector((0.21, 0.06, -0.15)),
    ], radius=0.014)
    hose("C002_Hose_R", "DEF-spine.003", [
        chest + Vector((-0.11, 0.30, 0.13)),
        chest + Vector((-0.24, 0.24, 0.02)),
        chest + Vector((-0.21, 0.06, -0.15)),
    ], radius=0.014)
    ...
    for obj in collection.objects:
        obj["steamtek_character_id"] = CHARACTER_ID
        obj["steamtek_production_stage"] = "detailed_prototype_v1"
        obj.hide_render = False
        if obj.type == "MESH":
            for polygon in obj.data.polygons:
                polygon.use_smooth = True
            bevel = obj.modifiers.new(name="Steamtek_SurfaceBevel", type="BEVEL")
            bevel.width = 0.004
            bevel.segments = 2
    ...
    bpy.context.scene["steamtek_character_status"] = "detailed_prototype_ready_for_direction_review"
    ...
    print(f"STEAMTEK_C002_DETAILED_PROTOTYPE={args.output.resolve()}")
