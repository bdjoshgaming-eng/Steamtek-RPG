...
const FACING_CALIBRATION_STEP := deg_to_rad(5.0)
const GRAVITY := 18.0
...
var model_forward_yaw_offset := 0.0
...
func _unhandled_key_input(event: InputEvent) -> void:
	...
	match key_event.physical_keycode:
		KEY_Q:
			model_forward_yaw_offset -= FACING_CALIBRATION_STEP
		KEY_E:
			model_forward_yaw_offset += FACING_CALIBRATION_STEP
		KEY_R:
			model_forward_yaw_offset = 0.0
...
func _update_animation_telemetry() -> void:
	...
	diagnostics_label.text = (
		...
		+ "Model-forward correction: %+.1f deg (Q/E adjust, R reset)\n" % rad_to_deg(model_forward_yaw_offset)
