#!/usr/bin/env python3
"""Extract source textures and render staged L4 sectional QA views."""
...
STAGE = ROOT / "incoming" / "meshy_apartment_assets" / "APT_Couch_L4_Left" / "staged_pipeline"
SOURCE = STAGE / "STK_PROP_Couch_L4_Left_ProductionCandidate.glb"
...
def extract_textures() -> None:
    ...
    for image in bpy.data.images:
        ...
        if "basecolor" in lower:
            stem = "STK_PROP_Couch_L4_Left_ProductionCandidate_Baked_BaseColor"
        elif "metallicroughness" in lower:
            stem = "STK_PROP_Couch_L4_Left_ProductionCandidate_Baked_MetallicRoughness"
        elif "emit" in lower:
            stem = "STK_PROP_Couch_L4_Left_ProductionCandidate_Baked_Emit"
        else:
            continue
    ...
    for material in bpy.data.materials:
        ...
        mr_node = next(
            (
                ...
                if node.type == "TEX_IMAGE" and node.image and "metallicroughness" in node.image.name.lower()
            ),
            None,
        )
        if mr_node is None:
        ...
        multiply.inputs[1].default_value = 1.20
...
def setup_scene() -> bpy.types.Object:
    ...
    world = bpy.data.worlds.new("Steamtek_Sectional_QA_World")
    ...
    add_area("Warm_Key", (3.8, -4.2, 3.6), 950.0, 3.4, (1.0, 0.70, 0.52))
    add_area("Cool_Fill", (-3.3, -2.0, 2.5), 650.0, 3.0, (0.38, 0.72, 1.0))
    add_area("Rear_Rim", (0.0, 3.2, 2.8), 520.0, 2.8, (0.28, 0.58, 0.82))
...
def render_views(camera: bpy.types.Object) -> None:
    scene = bpy.context.scene
    views = {
        "Front": ((0.0, -5.2, 0.55), (0.0, 0.0, 0.45), 3.75, (1100, 620)),
        "Back": ((0.0, 5.2, 0.55), (0.0, 0.0, 0.45), 3.75, (1100, 620)),
        "Left": ((-5.0, 0.0, 0.55), (0.0, 0.0, 0.45), 2.20, (720, 720)),
        "Right": ((5.0, 0.0, 0.55), (0.0, 0.0, 0.45), 2.20, (720, 720)),
        "Top": ((0.0, 0.0, 5.0), (0.0, 0.0, 0.25), 3.75, (1100, 720)),
    }
    for name, (location, target, scale, resolution) in views.items():
        ...
        scene.render.filepath = str(PREVIEWS / f"STK_PROP_Couch_L4_Left_Candidate_{name}.png")
        bpy.ops.render.render(write_still=True)

    camera.data.type = "PERSP"
    camera.data.lens = 58
    camera.location = (4.0, -4.6, 2.8)
    look_at(camera, (0.0, 0.0, 0.45))
    scene.render.resolution_x = 1000
    scene.render.resolution_y = 800
    scene.render.filepath = str(PREVIEWS / "STK_PROP_Couch_L4_Left_Candidate_ThreeQuarter.png")
    bpy.ops.render.render(write_still=True)
...
def main() -> None:
    ...
    render_views(setup_scene())
    print(f"Rendered sectional previews to {PREVIEWS}")
